...
$fieldSort = array("id_supplier","nama","alamat","telpon");
$fieldSearch = array("kode_supplier","nama","alamat","telpon");
$stringSearch = "";
...
if($act == "getAll"){
	...
	$query = $cls->getArray("*", "supplier", "($criteria) ORDER BY $fieldSort[$sortBy] $sortDir LIMIT $offset, $perPage");
	$get_count = $cls->getCount("*", "supplier", "($criteria)");
	...
	$query = $cls->getArray("*", "supplier", "id_supplier=".$id);
	...
	if($id == 0){
		$query = $cls->addNew("supplier", $data_array);
		$getLast = $db->lastInsertId();

		$setKode = array(
			"kode_supplier" => $kode_prefix.$getLast
		);

		$cls->update("supplier", $setKode, "id_supplier=".$getLast);
		
	}else{
		$query = $cls->update("supplier", $data_array, "id_supplier=".$id);
	}
	...
	$query = $cls->del("supplier", "id_supplier=".$id);
